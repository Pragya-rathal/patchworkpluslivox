...
struct Params {
  ...
  // Traversability
  double max_traversable_slope_deg;
  double roughness_threshold;
  double vegetation_threshold;
  int min_patch_points;
  double terrain_patch_size;
  bool publish_terrain_normals;
...
class PatchWorkpp {
 public:
  ...
  Eigen::MatrixX3f getTraversableGround() { return toEigenCloud(cloud_traversable_ground_); }
  Eigen::MatrixX3f getNontraversableGround() { return toEigenCloud(cloud_nontraversable_ground_); }
  Eigen::MatrixX3f getTerrainNormals() { return toEigenCloud(terrain_normals_); }

  void analyze_traversability();
 ...
 private:
  ...
  vector<PointXYZ> cloud_traversable_ground_, cloud_nontraversable_ground_;

  vector<PointXYZ> centers_, normals_;
  vector<PointXYZ> terrain_normals_;
